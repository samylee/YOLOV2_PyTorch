...
def load_darknet_weights(model, weights_path):
    ...
    # bias
    num_b = output_layer.bias.numel()
    output_b = torch.from_numpy(weights[ptr: ptr + num_b]).view_as(output_layer.bias)
    output_layer.bias.data.copy_(output_b)
    ptr += num_b
    # weights
    num_w = output_layer.weight.numel()
    output_w = torch.from_numpy(weights[ptr: ptr + num_w]).view_as(output_layer.weight)
    output_layer.weight.data.copy_(output_w)
    ptr += num_w
